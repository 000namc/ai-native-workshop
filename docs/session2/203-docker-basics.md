...
```
[Dockerfile]           [Image]              [Container]
환경을 정의한          빌드된 결과물          실행 중인 환경
텍스트 파일     →      (스냅샷)        →     (실제 동작)
```

| 단계 | 비유 | 명령어 |
|------|------|--------|
| Dockerfile | 레시피 (재료 + 순서) | — |
| Image | 레시피대로 만든 밀키트 (포장된 상태) | `docker build` |
| Container | 밀키트를 조리한 요리 (실행 중) | `docker run` |

- **Dockerfile**을 작성하면 → `docker build`로 **Image**가 만들어지고
- **Image**를 실행하면 → `docker run`으로 **Container**가 됩니다
- 하나의 Image로 여러 Container를 띄울 수 있습니다
